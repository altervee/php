...
<html>
    ...
    <?php
        ...
        $consultaCategorias = mysqli_query($conexion, "select name from category");// selecionamos directamente
        $numCategorias = mysqli_num_rows($consultaCategorias);//contamos el nuemro de rowa
    ?>
    <body>
        <form action="listado.php" method="post">
            <select id="elegirGenero" name="elegirGenero"><!-- aqui selecionamis la categoria segun el numero -->
                <?php
                    for ($i = 0; $i < $numCategorias; $i++) {
                        $categoria = mysqli_fetch_row($consultaCategorias);
                        echo "<option value='".($i+1)."'>".$categoria[0]."</option>";//generamos los generos
                    }
                ?>
            </select>
    ...
    <?php
        if(isset($_POST["mostrar"])) {//en el caso que se seleccione mostrar
            ...
            $numPeliculasPorGenero = mysqli_num_rows($consultaPeliculasPorGenero);//volvemos a recoplar el numero de rows
